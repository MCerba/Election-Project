 ...
 * goal is to ensure that all concrete objects are compatible; i.e., a DawsonElection may have
 * different rules and therefore require different implementation than a VanierElection.
 * 
 * To ensure that we program to interfaces instead of implementation, all objects should be
 * instantiated through the Factory object. Only objects in test classes should use the concrete
...
public interface ElectionFactory extends Serializable {
   ...
   * @return
   */
  Election getElectionInstance(String name, String type, int startYear, int startMonth,
  ...
  /**
   * Instantiates a Tally object with the results-to-date and associates to an Election
   * 
   * @param results
   * @param election
   */
