...
public class SequentialTextFileList implements ListPersistenceObject {
   ...
   * the Voter, Elections, and Tallies.
   * 
   ...
   */
  public SequentialTextFileList(String voterFilename, String electionFilename,
      String tallyFilename) {
  ...
  public List<Voter> getVoterDatabase() {
    Voter[] voters;
    try {
      voters = ElectionFileLoader.getVoterListFromSequentialFile(this.voterFilename);
    } catch (IOException e) {
   ...
   * ArrayList of size zero will be returned.Once the Election objects are created, the Tallies
  ...
  public List<Election> getElectionDatabase() {
    ...
    try {
      elections = ElectionFileLoader.getElectionListFromSequentialFile(this.electionFilename);
