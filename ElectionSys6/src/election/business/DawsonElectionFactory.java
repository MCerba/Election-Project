 ...
 * as many objects as there are enum constants - in this case only one. As such, the factory is a
 * singleton. The factory provides methods top centralize the instantiation of all concrete classes
 * related to the Dawson system. This facilitates the rest of the code being programmed to
...
public enum DawsonElectionFactory implements ElectionFactory {
    ...
    @Override
    public Voter getVoterInstance(String firstName, String lastName, String email,
                                  String postalcode) {
    ...
    @Override
    public Election getElectionInstance(String name, String type, int startYear, int startMonth,
                                        int startDay, int endYear, int endMonth, int endDay, String startRange, String endRange,
                                        String... choices) {
        return new DawsonElection(name, type, startYear, startMonth, startDay, endYear, endMonth,
                endDay, startRange, endRange, new DawsonTally(choices.length, name), choices);
    }

    @Override
    public Election getElectionInstance(Election e) {
        return new DawsonElection(e.getName(), e.getElectionType().name(), e.getStartDate().getYear(),
                e.getStartDate().getMonthValue(), e.getStartDate().getDayOfMonth(),
                e.getEndDate().getYear(), e.getEndDate().getMonthValue(), e.getEndDate().getDayOfMonth(),
                e.getPostalRangeStart(), e.getPostalRangeEnd(),
                new DawsonTally(e.getElectionChoices().length, e.getName()), e.getElectionChoices());
    }
