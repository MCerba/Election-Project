...
public class TextController {
  ...
  private Voter voterInfo(Scanner keyboard) {
    ...
    // get Voter info
    try {
      voter = model.findVoter(email);
    } catch (InexistentVoterException e) {
      System.out.println("\nNo Voter found with that email!");
  ...
  private void newVoter(Scanner keyboard) {
    ...
    String firstname = getInput(keyboard, "Please enter the first name: ");
  ...
  private void getWinner(Scanner keyboard) {
    ...
    Election el = this.getElection(keyboard);
    try {
      model.getWinner(el);
    } catch (IncompleteElectionException e) {
      System.out.println("Election is not yet complete!\n");
      System.out.println("Please wait until it is over.");
    }
  ...
  private Election getElection(Scanner keyboard) {
    // keyboard.nextLine(); // consume any previous value
